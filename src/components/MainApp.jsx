...
import './styles/MainApp.css'; // Import the CSS file
const initialBooks = [
  {
    id: '1',
...
export function MainApp() {
  ...
  const filteredBooks = useMemo(() => {
    return books.filter(({ title, author, description, tags, category: cat, status: stat, language: lang, rating }) => {
      const matchSearch = [title, author, description].some(field => field.toLowerCase().includes(search.toLowerCase())) ||
  ...
  const handleAdd = (book) => {
    const newBook = { ...book, id: Date.now().toString(), dateAdded: new Date().toISOString(), lastModified: new Date().toISOString() };
  ...
  const handleEdit = (book) => {
    setBooks(prev =>
      prev.map(b =>
        b.id === editingBook.id ? { ...book, id: b.id, dateAdded: b.dateAdded, lastModified: new Date().toISOString() } : b
      )
    );
    setEditingBook(null);
  };

  const handleDelete = (id) => window.confirm('Delete this book?') && setBooks(prev => prev.filter(b => b.id !== id));
  const handleStatus = (id, status) => setBooks(prev => prev.map(b => b.id === id ? { ...b, status, lastModified: new Date().toISOString() } : b));
  const handleLoan = (id, loanInfo) => setBooks(prev => prev.map(b => b.id === id ? { ...b, status: 'Loaned', loanInfo, lastModified: new Date().toISOString() } : b));
        ...
        {showFilters && (
          <div className="filters-grid">
            {[{ label: 'Category', value: category, set: setCategory, options: categories },
              ...
              .map(({ label, value, set, options }) => (
                <div key={label} className="filter-group">
                  <label>{label}</label>
                  <select value={value} onChange={(e) => set(e.target.value)} className="filter-select">
            ...
            <div className="filter-group">
              ...
              <input type="number" min="0" max="5" step="0.5" value={minRating} onChange={e => setMinRating(+e.target.value)} className="filter-input" />
            </div>
          </div>
        )}
        ...
        <div className="books-grid">
          {filteredBooks.map(book => (
            <BookCard
              key={book.id}
              book={book}
              onEdit={user?.role === 'admin' ? () => { setEditingBook(book); setShowForm(true); } : undefined}
              onDelete={user?.role === 'admin' ? handleDelete : undefined}
              onStatusChange={user?.role === 'admin' ? handleStatus : undefined}
              onLoan={user?.role === 'admin' ? () => setLoaningBook(book) : undefined}
            />
          ))}
        </div>

        {filteredBooks.length === 0 && <p className="no-books-message">No books found.</p>}
      </main>
